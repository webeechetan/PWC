                        ...
                        <div class="row align-items-end">
                            ...
                            <div>
                                @foreach($seasons as $season)
                                    @if($season->is_active)
                                        <a href="{{ route('admin.startup.list') }}" class="btn btn-success btn-sm">{{ $season->season }}</a>
                                    @else
                                        <a href="{{ route('admin.startup.list.with.season',$season->id) }}" class="btn btn-warning btn-sm">{{ $season->season }}</a>
                                    @endif
                                @endforeach
                            </div>
                        </div>
...
@section('script')
    ...
    const Columns = [{
        ...
        formatter: (value, row, index) => {
            return `<div style="width: 210px;"><input type="radio" value="1" name="request${row.id}" onclick="changeApproved(${row.id}, this)" ${row.request === 1 ? "checked" : ''}> <small>Interested</small> &nbsp; &nbsp; 
                        <br/><input type="radio" value="2" name="request${row.id}" onclick="changeApproved(${row.id}, this)" ${row.request === 2 ? "checked" : ''}> <small>Not Interested</small>
                    </div>`;
        }
    }, {
        ...
        field: 'Result after 1st physical meeting',
        title: 'Result after 1st physical meeting',
        align: 'center',
        sortable: true,
        formatter: (value, row, index) => {
    ...
    initializeDatatable({
        Url: '{{env('APP_URL')}}/admin/startup?request=table{{$dateFilter}}{{$aditionalFilter}}&use_scope={{$use_scope}}&season_id={{$season_id}}',
        Columns: Columns,
    })
}
const removeEval = (d) => {
